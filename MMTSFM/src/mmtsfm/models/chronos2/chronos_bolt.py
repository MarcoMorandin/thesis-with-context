...
class InstanceNorm(nn.Module):
    ...
    def __init__(self, eps: float = 1e-3, use_arcsinh: bool = False) -> None:
        super().__init__()
        # eps acts as a hard scale floor. 1e-3 keeps (x-loc)/scale ≤ ~1e3 for inputs
        # in [-1, 1] range — well inside fp32 arcsinh's stable region. The previous
        # 1e-5 default amplified near-constant context windows by 1e5×, producing
        # finite-but-huge normalized targets that drove loss to ~1e33.
        self.eps = eps
    ...
    def forward(
        ...
        if loc_scale is None:
            ...
            # Floor scale at eps for tiny/non-finite values (not just exact 0). Denormal
            # or near-zero std on near-constant context windows would otherwise blow up
            # the division below.
            scale = torch.where(torch.isfinite(scale) & (scale > self.eps), scale, torch.full_like(scale, self.eps))
        ...
        if self.use_arcsinh:
            ...
            # When arcsinh is disabled, (x-loc)/scale is unbounded. Clip to a
            # generous range so a single bad sample cannot overflow the loss.
            scaled_x = scaled_x.clamp(min=-1e4, max=1e4)

        scaled_x = torch.nan_to_num(scaled_x, nan=0.0, posinf=0.0, neginf=0.0)
...
# Vendored copy — differs from layers.ResidualBlock (upstream variant). Do not merge.
    ...
    def predict_quantiles(
        ...
        **predict_kwargs,
        ...
        if set(quantile_levels).issubset(set(training_quantile_levels)):
            ...
            # Vendored from Amazon AutoGluon (Apache-2.0). Known approximation: assumes the model's
            # quantiles during training match training_quantile_levels. Acceptable for current usage.
