...
class Chronos2CoreConfig(PretrainedConfig):
    ...
    grassmann_reduced_dim
        Dimension r for linear reduction in Grassmann mixing, by default 32
    grassmann_window_offsets
        Multi-scale causal window offsets for Grassmann pairing, by default [1, 2, 4, 8, 12, 16]
    grassmann_plucker_eps
        Epsilon for Plücker vector normalization, by default 1e-8
    grassmann_modality_pair_bias
        If True, adds 4 learned scalar biases (TT/TV/VT/VV pair types) to the
        Grassmann offset logits. Active only when fusion_mode="interleaved".
        Default True.
    """
    ...
    def __init__(
        ...
        use_grassmann: bool = True,
        grassmann_modality_pair_bias: bool = True,
        **kwargs,
    ):
        self.use_grassmann = use_grassmann
        self.grassmann_modality_pair_bias = grassmann_modality_pair_bias
