...
from .grassmann import CausalGrassmannMixing
from .layers import (
    MHA,
    MLP,
    AttentionOutput,
...
class Chronos2EncoderBlock(nn.Module):
    def __init__(
        ...
        if getattr(config, "use_grassmann", True):
            self.layer.append(CausalGrassmannMixing(config))
        else:
    ...
    def forward(
        ...
        *,
        ...
        # backward, which would poison the PARAMETER gradients of this block.
...
class Chronos2Encoder(nn.Module):
    ...
    def _expand_and_invert_time_attention_mask(
        ...
        # Always compute in float32: finfo(float16).min = -65504 (finite); when scale=1.0
        # is used without head-dim normalization, logits can overflow float16 causing
        # CUDA scatter-gather assertion failures. float32 finfo.min ≈ -3.4e38 is safe.
    ...
    def _construct_and_invert_group_time_mask(
        ...
        # Always compute the additive mask in float32 to avoid float16 saturation.
        # finfo(float16).min = -65504 is a finite value; when added to large logits
        # under scale=1.0 it can overflow. float32 finfo.min ≈ -3.4e38 is safe.
        group_time_mask = rearrange(group_time_mask, "q b t -> t 1 q b")
        group_time_mask = (1.0 - group_time_mask.float()) * torch.finfo(
            torch.float32
        ).min
    ...
    def forward(
        ...
        *,
        ...
        for i, (layer_module) in enumerate(self.block):
            ...
            # Guard: if a block produces NaN (e.g. from group-attention softmax
            # on near-empty groups), clean it here so the NEXT block gets clean
            # input and its backward does not propagate NaN back to all_embeds.
            # nan_to_num on hidden_states (not a clamp) only zeroes NaN/inf
            # positions — finite activations pass through with full gradients.
...
class Chronos2Model(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, CausalGrassmannMixing):
            # Xavier init for reduction and projection, zeros for gate bias
            d_model = self.config.d_model
            r = self.config.grassmann_reduced_dim
            plucker_dim = r * (r - 1) // 2
            module.W_red.weight.data.normal_(mean=0.0, std=factor * (d_model**-0.5))
            if module.W_red.bias is not None:
                module.W_red.bias.data.zero_()
            module.W_plu.weight.data.normal_(mean=0.0, std=factor * (plucker_dim**-0.5))
            if module.W_plu.bias is not None:
                module.W_plu.bias.data.zero_()
            module.W_gate.weight.data.normal_(
                mean=0.0, std=factor * ((2 * d_model) ** -0.5)
            )
            if module.W_gate.bias is not None:
                module.W_gate.bias.data.zero_()
            # offset_weights / modality_pair_bias are MISSING from the pretrained
            # checkpoint; under from_pretrained's meta+to_empty() materialization
            # they only get values here. Skipping them leaves uninitialized GPU
            # garbage (observed as random NaN that permanently froze training).
            module.offset_weights.data.fill_(1.0)
            if getattr(module, "modality_pair_bias", None) is not None:
                module.modality_pair_bias.data.zero_()
    ...
    def _compute_loss(
        ...
        # Defensive: even with arcsinh + scale floor, ensure no rogue value escapes.
        # Loss is computed against quantile_preds which is clamped to [-20, 20] upstream
        # (vision_chronos2.py); matching the target range here keeps |target - pred| ≤ ~40.
        future_target = torch.nan_to_num(future_target, nan=0.0, posinf=0.0, neginf=0.0)
        future_target = future_target.clamp(min=-20.0, max=20.0)
        future_target = future_target.to(torch.float32)
        quantile_preds = quantile_preds.to(torch.float32)
        ...
        loss_val = quantile_loss * loss_mask

        # FIX H: masked mean over horizon — divide only by unmasked positions.
        # Plain .mean(dim=-1) divides by total (padded) H; with horizon=12 and
        # output_patch_size=8 the denominator is 16, under-weighting gradient by 25%.
        n_unmasked = loss_mask.sum(dim=-1).clamp(min=1.0)  # [B, 1]
        loss = (loss_val.sum(dim=-1) / n_unmasked).sum(dim=-1).mean()
    ...
    def forward(
        ...
        reg_len = 1 if self.chronos_config.use_reg_token else 0
        assert hidden_states.shape == (
            batch_size,
            num_context_patches + reg_len + num_output_patches,
            self.model_dim,
        )
