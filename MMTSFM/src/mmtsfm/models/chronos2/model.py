    ...
    def forward(
        ...
        *,
        ...
        if hidden_states.requires_grad:
            hidden_states.register_hook(lambda g: torch.nan_to_num(g, nan=0.0, posinf=0.0, neginf=0.0))
...
class Chronos2Encoder(nn.Module):
    ...
    def _construct_and_invert_group_time_mask(
        ...
        group_time_mask = (1.0 - group_time_mask.float()) * torch.finfo(torch.float32).min
...
class Chronos2Model(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Chronos2LayerNorm):
            ...
            module.W_gate.weight.data.normal_(mean=0.0, std=factor * ((2 * d_model) ** -0.5))
    ...
    def forward(
        ...
        assert hidden_states.shape == (batch_size, num_context_patches + reg_len + num_output_patches, self.model_dim)
