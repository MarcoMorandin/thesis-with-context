...
the canonical `image_h5_index`.
    ...
    item = ds[0]            # "V": (T, 1, 64, 64); "mask_visual": (T,)
"""
...
def _to_unix_seconds(ts: pd.Series) -> np.ndarray:
    ...
    if t.tz is None:
        t = t.tz_localize("UTC")
    return (
        ((t - pd.Timestamp(0, tz="UTC")) // pd.Timedelta("1s"))
        .to_numpy()
        .astype(np.int64)
    )
...
class UKMultimodalDataset:
    ...
    attaches, per window, the satellite frames over the latest requested visual
    history (T_v <= history): `V` (T_v, C, S, S) in [0,1] and `mask_visual`
    (T_v,) — 1 where a frame exists on that step (daylight), 0 otherwise.
    ...
    def __init__(
        ...
        to_gray: bool = True,
        visual_history_steps: int | None = None,
    ):
        ...
        if visual_history_steps is not None and visual_history_steps < 1:
            raise ValueError("visual_history_steps must be positive")
        self.visual_history_steps = min(visual_history_steps or history, history)
        ...
        frame_df = df[df[FRAME_IDX_COL].notna() & (df[FRAME_IDX_COL] >= 0)]
        if frame_df.empty:
        ...
        for (ds, site), g in frame_df.groupby([config.DATASET_COL, config.SITE_COL]):
            key = (str(ds), str(site))
            self.coords[key] = (
                float(g["latitude"].iloc[0]),
                float(g["longitude"].iloc[0]),
            )
