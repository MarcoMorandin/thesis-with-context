...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        # !预测头
        # original: 一般都有一个decoder_pred
        self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size**2 * in_chans, bias=True)  # decoder to patch
        if self.quantile:
            # adds quantile outputs:
            # ! we need 3 outputs, corresponding to 50%, 25% and 75%
            # self.quantile_num = 3
            # self.decoder_pred_1 = nn.Linear(decoder_embed_dim, patch_size**2 * in_chans, bias=True)  # decoder to patch
            # 如果quantile预测再额外加两个：
            self.decoder_pred_2 = nn.Linear(decoder_embed_dim, patch_size**2 * in_chans, bias=True)  # decoder to patch
            self.decoder_pred_3 = nn.Linear(decoder_embed_dim, patch_size**2 * in_chans, bias=True)  # decoder to patch
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        if not self.quantile:
            ...
            # x_mean = self.decoder_pred_1(x)[:, 1:, :]
            x_mean = self.decoder_pred(x)[:, 1:, :]
            x_25 = self.decoder_pred_2(x)[:, 1:, :]
            x_75 = self.decoder_pred_3(x)[:, 1:, :]
            # quantile情况下，生成50、25和75三组数据
            return x_mean, x_25, x_75
