...
def time_features_from_freq_str(freq_str: str) -> List[TimeFeature]:
    ...
    # Normalize deprecated aliases ('t' → 'min', 'T' → 'min')
    freq_str = {'t': 'min', 'T': 'min'}.get(freq_str, freq_str)
    offset = frequencies.to_offset(freq_str)
...
def time_features(dates: pd.DatetimeIndex, freq: str = 'h') -> np.ndarray:
    return np.vstack([feat(dates) for feat in time_features_from_freq_str(freq)])
