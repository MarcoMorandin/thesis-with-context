...
# from retrieve_Z import load_database

from retrieve_X import do_retrieve as do_retrieve_X
from retrieve_X import load_database
...
SPACE = os.environ.get("RETRIEVE_SPACE", "")
from models.CrossRAG import ChronosBoltModelForForecastingWithRetrieval  
...
parser.add_argument('--freq', type=int, default=0)  # 0 -> 'h' (int 1 is not a valid pandas offset)
...
parser.add_argument('--alpha', type=float, default=100.0, help='percentage of train set to use for retrieval (default: 100.0 for full train set)')
...
if not os.path.exists(best_model_path):
    print('no corresponding checkpoint!!')
...
if 'retrieve' in args.model_id:
    ...
    if os.path.exists(retrieved_data_path):
        ...
        do_retrieve_X(ori_data_path.split('.')[0], args.retrieval_database_dir, args.root_path, args.metadata, 
                      args.mode, top_k, args.seq_len, args.pred_len, fix_seed, save=args.save)
#         do_retrieve_Z(ori_data_path.split('.')[0], args.retrieval_database_dir, args.root_path, args.metadata, 
#                       args.mode, top_k, args.seq_len, args.pred_len, fix_seed, args.dimension, embedding_model, args.save, args.embedding_tuning)
    ...
    db_suffix = f'_{SPACE}_space.pkl'
...
#_Z_space.pkl    
    if args.mode == 'only_self' or args.mode == 'only_self_train':
        ...
        if not os.path.exists(db_path):
            base_db_path = os.path.join(args.retrieval_database_dir, f'{args.metadata["database_name"][0]}_{args.metadata["frequency"]}_{args.metadata["lookback_length"]}{db_suffix}')
        ...
        for database_name in args.metadata['database_name']:
            ...
            if not os.path.exists(db_path) and db_suffix == '_Z.pkl':
                base_db_path = os.path.join(args.retrieval_database_dir, f'{database_name}_{args.metadata["frequency"]}_{args.metadata["lookback_length"]}{db_suffix}')
                if os.path.exists(base_db_path):
                    db_path = base_db_path
            database = load_database(db_path)
            for variable in database.keys():
...
log_dir = 'results_shift/forecast_evaluation' if is_shift else 'results/forecast_evaluation'
