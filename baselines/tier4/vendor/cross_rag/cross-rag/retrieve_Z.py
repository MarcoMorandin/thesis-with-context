...
def generate_retrieval_database(dataset_name, lookback_length, embedding_model, database_dir, root_dir):
    ...
    save_database(databases, os.path.join(database_dir, f'{dataset_name}_{frequency}_{lookback_length}_Z_space.pkl'))
...
class Retriever():
    ...
    def build_index(self, y_length, begin=None, end=None, variable_filter=None):
        ...
        for database_name in self.metadata['database_name']:
            # Z-space database file with _Z suffix
            database_path = f'{database_name}_{self.metadata["frequency"]}_{self.metadata["lookback_length"]}_Z_space.pkl'
            ...
            if os.path.exists(db_full_path):
                # Check if database has correct format (Z-space: should have 'embeddings' key)
                try:
                    test_database = load_database(db_full_path)
                    # Check if it's Z-space format (has 'embeddings' for at least one variable)
                    has_correct_format = False
                    for test_key in test_database.keys():
                        if isinstance(test_database[test_key], dict) and 'embeddings' in test_database[test_key]:
                            has_correct_format = True
                            break
                    if not has_correct_format:
                        print(f'{database_path} exists but has wrong format. Regenerating as Z-space format...')
...
def do_retrieve(original_data_name, retrieval_database_dir, root_dir, metadata, mode, top_k, context_length, prediction_length, seed, dimension, embedding_model, save=True, embedding_tuning=None, retrieve_suffix=""):
    '''
    ...
    if save:
        ...
        retrieved_data_path = os.path.join(root_dir, f'{original_data_name}_retrieve_{retrieval_database_names}_{metadata["lookback_length"]}_{mode}_{embedding_tuning}{suffix}.csv')
