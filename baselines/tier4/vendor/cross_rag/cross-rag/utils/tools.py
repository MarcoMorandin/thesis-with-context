...
def test(model, test_data, test_loader, args, device):
    ...
    preds = preds.reshape(-1, preds.shape[-2], preds.shape[-1])
    trues = trues.reshape(-1, trues.shape[-2], trues.shape[-1])
    # align prediction/target lengths (use the shorter length)
    common_len = min(preds.shape[-1], trues.shape[-1])
    if preds.shape[-1] != trues.shape[-1]:
        preds = preds[..., :common_len]
        trues = trues[..., :common_len]
...
def test_retrieve(model, test_data, test_loader, args, device):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, retrieved_seqs, distances) in tqdm(enumerate(test_loader)):
            ...
            # Preserve batch dimension; only squeeze channel dim if present
            batch_x = batch_x.float().to(device)
            batch_y = batch_y.float().to(device)
            ...
            if args.model == 'ChronosBoltRetrieve':
                ...
                if USE_SOFT_WEIGHT and not USE_TABPFN and hasattr(args, "temperature"):
                    forward_kwargs["temperature"] = getattr(args, "temperature", 1.0)
