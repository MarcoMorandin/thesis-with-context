...
\hline
Chronos-2 patch embedding & \textbf{reinitialised} & trainable throughout \\
Group self-attention & pretrained & last 3 of 6 blocks trainable at $0.1\times$ LR \\
Feed-forward & pretrained & last 3 of 6 blocks trainable at $0.1\times$ LR \\
Temporal mixer ($\times 6$) & \textbf{from scratch} (Grassmann) & trainable throughout \\
...
thereafter held fixed \emph{except} its last three of six encoder blocks, which continue to
train at $0.1\times$ the base learning rate together with the re-initialised patch
embeddings. Two consequences follow, and both are stated rather than elided. The weights
held fixed are not the pretrained ones but a PV-adapted checkpoint; and half the encoder is
still adapting while the visual path is learned. Whether the visual path in fact
\emph{needs} that adaptation capacity --- that is, whether a genuinely frozen backbone
($n_{\text{unfreeze}} = 0$) reads the sky as well --- is measured directly by the ablation
...
backbone fixed apart from its top three encoder blocks, which train at $0.1\times$ LR, while
