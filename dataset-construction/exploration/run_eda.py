...
IMAGES_H5_PATH = os.path.join(DATA_DIR, "images_all.h5")
...
# images live in a single HDF5 file, one group per site keyed "<dataset>_<site_id>",
# row indexed by the parquet's image_index column. uk_pv = (128,128) grayscale,
# goes_pvdaq = (256,256,3) RGB.
IMG_H5 = h5py.File(IMAGES_H5_PATH, "r")


def img_key(row):
    return f"{row.dataset}_{row.site_id}"


def img_array(row):
    """Pixel array for a row, or None if the index is unavailable."""
    key = img_key(row)
    if key not in IMG_H5:
        return None
    ds = IMG_H5[key]["images"]
...
def img_gray(arr):
    """Luminance float32 array from a packed image array."""
    if arr.ndim == 3:
        return arr[..., :3].astype(np.float32) @ np.array([0.299, 0.587, 0.114],
                                                          dtype=np.float32)
    return arr.astype(np.float32)


def img_meta(arr):
    """(width, height) size tuple and PIL-style mode string."""
    h, w = arr.shape[:2]
    mode = "RGB" if arr.ndim == 3 else "L"
    return (w, h), mode


def img_timestamp(row):
    key = img_key(row)
    if key not in IMG_H5:
        return None
    ts = IMG_H5[key]["timestamps"]
    idx = int(row.image_index)
    if idx < 0 or idx >= ts.shape[0]:
        return None
    return ts[idx].decode()
...
# curation deliberately introduces NaNs; cross-check each against its documented
# cause so the report only warns on *unexplained* missingness.
masked_target = (df.get("outage_flag", False).fillna(False)
                 | df.get("stuck_flag", False).fillna(False))
clearsky_floor = df.get("clearsky_ghi", pd.Series(np.nan, index=df.index)) < 50
intended_nan = {
    "power_w": masked_target,
    "norm_power": masked_target,
    "kt": clearsky_floor,
    "csi": clearsky_floor | masked_target,
}
...
if missing_df.empty:
    ...
    for col in missing_df.index:
        ...
        if col in intended_nan:
            unexplained = int((nan_mask & ~intended_nan[col]).sum())
            cause = {
                "power_w": "masked outage/stuck rows (curation)",
                "norm_power": "masked outage/stuck rows (curation)",
                "kt": "clear-sky GHI < 50 W/m² (undefined)",
                "csi": "clear-sky GHI < 50 W/m² or masked power (undefined)",
            }[col]
            causes.append(f"intentional — {cause}"
                          + (f"; **{unexplained:,} UNEXPLAINED**" if unexplained else ""))
            if unexplained:
                add_issue("warn", f"`{col}` has {unexplained:,} missing values not "
                                  f"explained by curation masks")
        else:
            causes.append("unexpected")
            add_issue("warn", f"`{col}` has {missing_df.loc[col, 'missing']:,.0f} "
                              f"missing values ({missing_df.loc[col, '%']}%)")
    missing_df["cause"] = causes
    add_md(missing_df.to_markdown())
    add_md()
    add_md("All NaNs above are introduced on purpose by `curate_dataset.py` "
           "(target masking on outage/stuck rows; kt/csi undefined below the "
           "clear-sky floor) and verified row-for-row against their cause — none "
           "are unexplained data loss.")
add_md()
...
nan_power_unexplained = int((df.power_w.isna() & ~masked_target).sum())
checks.append((f"NaN power values ({nan_power - nan_power_unexplained:,} are intended "
               f"outage/stuck masks)", nan_power_unexplained,
               "warn" if nan_power_unexplained else None))
...
flagged_sites = set(df.loc[df.get("bad_site_flag", False).fillna(False), "site_id"])
if len(bad_sites):
    already = bad_sites.site_id.isin(flagged_sites)
    bad_sites = bad_sites.assign(bad_site_flag=already.values)
    add_md(f"**{len(bad_sites)} sites with corr < 0.6** "
           f"({int(already.sum())} already carry `bad_site_flag` from curation):")
    add_md()
    add_md(bad_sites.round(3).to_markdown(index=False))
    unflagged = bad_sites[~already.values].site_id.tolist()
    if unflagged:
        add_issue("error", f"{len(unflagged)} low-corr sites NOT flagged by curation: "
                           f"{unflagged}")
    else:
        add_md()
        add_md("All low-correlation sites are already flagged `bad_site_flag` by "
               "`curate_dataset.py` — exclude or down-weight them downstream. "
               "No unhandled coherence failures.")
else:
...
uk_all = day_df[(day_df.dataset == "uk_pv") & day_df.norm_power.notna()]
uk = uk_all.sample(min(120_000, len(uk_all)), random_state=RNG)
goes = day_df[(day_df.dataset == "goes_pvdaq") & day_df.norm_power.notna()]
...
# integrity: index resolvability + readability on a large sample
n_img_check = 3000
...
for _, row in img_sample.iterrows():
    arr = img_array(row)
    if arr is None:
        missing_imgs.append((img_key(row), int(row.image_index)))
        continue
    if arr.size == 0 or not np.isfinite(arr).all():
        corrupt_imgs.append((img_key(row), int(row.image_index)))
add_md(f"- Index-resolvability/readability check on {n_img_check:,} random rows: "
       f"**{len(missing_imgs)} unresolvable**, **{len(corrupt_imgs)} corrupt**.")
if missing_imgs:
    add_issue("error", f"{len(missing_imgs)}/{n_img_check} sampled rows have no image in "
                       f"images_all.h5 (e.g. {missing_imgs[:3]})")
if corrupt_imgs:
    add_issue("error", f"{len(corrupt_imgs)}/{n_img_check} sampled images corrupt")

# timestamp alignment: the h5 timestamp at image_index must match the row timestamp
mis_aligned, checked = 0, 0
for _, row in img_sample.head(500).iterrows():
    h5ts = img_timestamp(row)
    if h5ts is None:
        continue
    checked += 1
    if h5ts != row.timestamp_utc.strftime("%Y-%m-%dT%H:%M:%SZ"):
        mis_aligned += 1
add_md(f"- h5-timestamp↔row alignment check on {checked} rows: **{mis_aligned} mismatches**.")
add_md()
if mis_aligned:
    add_issue("error", f"{mis_aligned}/{checked} image timestamps do not match row timestamp")
...
for ds, g in df.groupby("dataset"):
    ...
    for _, row in s.iterrows():
        ...
        if arr is None:
            continue
        gray = img_gray(arr)
        size, mode = img_meta(arr)
        prop_rows.append({
            "dataset": ds, "site_id": row.site_id, "size": size,
            "mode": mode, "brightness": gray.mean(), "contrast": gray.std(),
