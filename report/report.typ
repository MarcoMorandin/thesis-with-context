...
The proposed transition from S2d to S2e extends this single-anchor design across the numerical context. Five visual anchors are placed beside Chronos-2 patches at the same clock time on consecutive days, giving the forecaster repeated sky--power pairs at matched solar geometry. The anchor stride is one day rather than one patch because an 8-hour ladder is unrealizable on this dataset: satellite frames exist only between 02:00 and 16:00 UTC, so a one-patch stride populates all five anchors for $0.0%$ of the $24{,}605$ evaluated origins, against $94.4%$ at 24 hours. Nothing is surrendered, since advection is exhausted by roughly two hours and the closer anchors were never nowcasting. The token budget is held near S2d's: 100 visual tokens rather than 98, but now split before ranking so each anchor draws 20 from its own $2 times 49 = 98$ candidates, since a global top-$k$ would cut one ranked list into five arbitrary groups. Sequence length is therefore $42 + 100 + 1 = 143$, against S2d's 141, and each visual block inherits its anchor patch's integer position identifier rather than S2d's fractional sub-patch scheme. Two follow-ups separate the remaining explanations while holding the five anchors fixed. A46 swaps the selector at the same 100-token budget: paired-novelty selection ranks the 49 two-latent spatial trajectories within each anchor and retains both endpoints of the ten that change most. A separate capacity control lifts the budget to 490, keeping all 98 tokens per anchor unpruned. Recovery under either would identify token allocation rather than interleaving as the failure mode.
